...
echo until mysql --host=${host} --user=${user} --password=${pass} --port=${port} ${name} -e "'SELECT 1;'" >>/log.txt
until mysql --host=${host} --user=${user} --password=${pass} --port=${port} ${name} -e "'SELECT 1;'" ; do date >>/log.txt; done
