...
В процесі роботи знайшов баг в імплементації параметру remember на ArchLinux - якщо remember>1, то перше введення нового паролю завжди буде давати помилку **пароль вже використовується**, незалежно від того, чи це насправді так. Більше того, такий пароль все одно запишеться в `/etc/security/opasswd`. Пароль можна буде змінити лише на одному із етапів `retry`. А якщо `retry=0`, то пароль не вдасться змінити взагалі
